...
const StudyMaterial = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:3001/api/generate-study-material", {
        method: "POST",
        headers: {
      ...
      <div className="flex bg-slate-100 p-1 rounded-xl w-full sm:w-fit mb-8 border border-slate-200">
        <button
          ...
          className={`flex-1 sm:flex-none px-6 md:px-10 py-2 rounded-lg font-bold text-xs md:text-sm transition-all ${
            view === "flashcards" ? "bg-white shadow-sm text-teal-600" : "text-slate-500"
          }`}
        >
          Flashcards
        </button>
        <button
          ...
          className={`flex-1 sm:flex-none px-6 md:px-10 py-2 rounded-lg font-bold text-xs md:text-sm transition-all ${
            view === "quiz" ? "bg-white shadow-sm text-teal-600" : "text-slate-500"
          }`}
        >
          Quiz
        </button>
                    ...
                    <div className="mt-4 space-y-2">
                      <p
                        className={`text-sm font-bold ${
                          selected === item.correctAnswer ? "text-green-600" : "text-red-600"
                        }`}
                      >
                        {selected === item.correctAnswer ? "Correct answer!" : "Wrong answer."}
                      </p>
                      <p className="text-sm text-slate-700">
                        <span className="font-bold">Your answer:</span> {selected}
                      </p>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-100">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-teal-50/30">
              <h2 className="text-lg md:text-xl font-bold text-teal-900">Auto-Generate</h2>
              <button
            ...
            <div className="p-6">
              <label className="relative border-2 border-dashed border-slate-200 rounded-2xl p-8 md:p-12 flex flex-col items-center justify-center hover:border-teal-400 hover:bg-teal-50/20 transition-all cursor-pointer group">
                ...
                <p className="font-bold text-slate-700 text-sm">
                  {isLoading ? "Reading PDF and generating questions..." : "Tap to upload PDF"}
                </p>
              </label>
              ...
              <div className="mt-6 bg-teal-50/50 border border-teal-100 p-4 rounded-xl flex gap-3">
                ...
                <p className="text-[11px] md:text-xs text-teal-800 leading-tight">
                  <b>Note:</b> AI will scan the PDF content and generate flashcards and quiz
